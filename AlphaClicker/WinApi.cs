...
namespace AlphaClicker
{
    public class WinApi
    {
        ...
        // Simulate mouse button press (down)
        public static void DoMouseDown(string button, bool useCustomCoords, int X, int Y)
        {
            if (useCustomCoords) { SetCursorPos(X, Y); }
            ...
            {
                case "Left":
                    mouse_event((uint)MOUSEEVENTF.LEFTDOWN, 0, 0, 0, 0);
                    break;
                case "Right":
                    mouse_event((uint)MOUSEEVENTF.RIGHTDOWN, 0, 0, 0, 0);
                    break;
                case "Middle":
                    mouse_event((uint)MOUSEEVENTF.MIDDLEDOWN, 0, 0, 0, 0);
                    break;
            }
        }

        // Simulate mouse button release (up)
        public static void DoMouseUp(string button)
        {
            switch (button)
            {
                case "Left":
